...
// Hero Banners — player-backend keys follow pattern: cms:banners:{platform}:{page}
const invalidateBannerCache = async () => {
  try {
    const keys = await redis.keys('cms:banners:*');
    if (keys.length > 0) {
      await redis.del(...keys);
      console.log('Banner cache invalidated:', keys.join(', '));
    }
  } catch (error) {
    console.error('Banner cache invalidation error:', error);
  }
};
