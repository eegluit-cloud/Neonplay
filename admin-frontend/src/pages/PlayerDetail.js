            ...
            {activeTab === 'transactions' && (
              <div className="transactions-panel">
                ...
                <div className="transactions-table">
                  <div className="table-header"><span>Ref</span><span>Type</span><span>Amount</span><span>Status</span><span>Date</span></div>
                  {filteredTransactions.length > 0 ? filteredTransactions.map(tx => (
                    <div key={tx.id} className="table-row">
                      <span className="tx-ref">{tx.reference?.substring(0, 12)}...</span>
                      <span className={`tx-type ${tx.type}`}>{tx.type}</span>
                      <span className="tx-amount">{tx.type === 'deposit' || tx.type === 'win' || tx.type === 'bonus' ? '+' : '-'}{formatCurrency(tx.amount)}</span>
