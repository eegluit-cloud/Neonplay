...
import { useGames, useGameProviders, type Game } from '@/hooks/useGames';
...
export function CategoryGamePage({ title, activeTab, category, filterFn, emptyMessage }: CategoryGamePageProps) {
  ...
  const { games: allGames, isLoading } = useGames(category ? { category, limit: 50 } : { limit: 50 });
  const { providers } = useGameProviders();

  // Apply optional client-side filter (e.g. isNew, isFeatured)
  const baseGames = filterFn ? allGames.filter(filterFn) : allGames;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className={`transition-all duration-300 pt-14 md:pt-16 pb-20 md:pb-0 ${sidebarOpen ? 'md:ml-56' : 'md:ml-16'}`}>
        <main className="p-3 md:p-4 lg:p-6 space-y-4 md:space-y-6 overflow-x-hidden max-w-full">
          ...
          <GameCategoryNav activeTab={activeTab} />
