...
const HotGames = () => {
  ...
  // Fetch hot games from API (isHot or isFeatured)
  const { games: allGames, isLoading } = useGames({ limit: 50 });
  const { providers } = useGameProviders();

  // Filter to hot/featured games
  const hotGames = allGames.filter(g => g.isHot || g.isFeatured);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className={`transition-all duration-300 pt-14 md:pt-16 pb-20 md:pb-0 ${sidebarOpen ? 'md:ml-56' : 'md:ml-16'}`}>
        <main className="p-3 md:p-4 lg:p-6 space-y-4 md:space-y-6 overflow-x-hidden max-w-full">
          ...
          <MobilePageHeader title={t('pages.hotGames')} />
          ...
          <ApiGameGrid
            ...
            emptyMessage="No hot games found"
          />
