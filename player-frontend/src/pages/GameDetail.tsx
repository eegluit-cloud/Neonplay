...
const GameDetail = () => {
  ...
  const handleLaunchGame = useCallback(async () => {
    ...
    setLaunchError(null);
    try {
      const response = await gamesApi.launchGame(gameId);
                  ...
                  {launchUrl ? (
                    ...
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <button
                        ...
                        disabled={launching}
                        className="relative z-10 w-20 h-20 flex items-center justify-center rounded-full bg-cyan-500 hover:bg-cyan-600 disabled:bg-cyan-500/50 transition-colors shadow-lg shadow-cyan-500/30"
                      >
                        {launching ? (
                      ...
                      <p className="relative z-10 text-white/80 text-sm">
                        {launching ? 'Loading game...' : 'Click to play'}
                      </p>
                      {launchError && (
                        <p className="relative z-10 text-red-400 text-sm max-w-md text-center">
                          {launchError}
                        </p>
                      )}
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      By <span className="text-cyan-400">{game.provider?.name}</span>
                    </p>
                  </div>
                  {game.rtp && (
